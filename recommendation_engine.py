...
def _normalize_selected_interest(value: Any) -> str:
...
def _parse_selected_interests(value: Any) -> list[str]:
    if value in (None, ""):
        return []
    ...
    filtered_values = [
        ...
        if (normalized := _normalize_selected_interest(item))
    ]
...
def _row_to_student(row: sqlite3.Row) -> StudentProfile:
    ...
    return StudentProfile(
        ...
        selected_interests=_parse_selected_interests(row["selected_interests"]),
...
def get_recommendations(
    ...
    with get_connection() as connection:
        ...
        prioritized_ranked = _rank_candidates(
            ...
            state_reference,
            state_reason_label,
            priority_interests=normalized_priority_interests,
        )
